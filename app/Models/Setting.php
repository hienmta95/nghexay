...
class Setting extends BaseModel
{
	...
	public function getValueAttribute($value)
	{
		...
		if (!empty($value)) {
			...
			if ($this->key == 'app') {
				foreach ($value as $key => $item) {
					if ($key == 'logo') {
						...
						if (!Storage::exists($value['logo'])) {
							$value[$key] = config('icetea.core.logo');
						}
					}

					if ($key == 'favicon') {
						if (!Storage::exists($value['favicon'])) {
							$value[$key] = config('icetea.core.favicon');
						}
					}
				}
				...
				if (!isset($value['logo'])) {
					$value['logo'] = config('icetea.core.logo');
				}
				if (!isset($value['favicon'])) {
					$value['favicon'] = config('icetea.core.favicon');
				}
				if (!isset($value['default_date_format'])) {
					$value['default_date_format'] = config('icetea.core.defaultDateFormat');
				}
				if (!isset($value['default_datetime_format'])) {
					$value['default_datetime_format'] = config('icetea.core.defaultDatetimeFormat');
				}
				if (!isset($value['default_timezone'])) {
					$value['default_timezone'] = config('icetea.core.defaultTimezone');
				}
			}
			...
			if ($this->key == 'single') {
				...
				if (!isset($value['guests_can_apply_jobs'])) {
					$value['guests_can_apply_jobs'] = '1';
				}
				if (!isset($value['simditor_wysiwyg'])) {
					$value['simditor_wysiwyg'] = '0';
				}
			}
			...
			if ($this->key == 'seo') {
				...
				if (!isset($value['multi_countries_urls'])) {
					$value['multi_countries_urls'] = config('icetea.core.multiCountriesUrls');
				}
			}
	...
	*/
	public function setValueAttribute($value)
	{
		...
		if (isset($value['logo'])) {
			$logo = [
				'attribute' => 'logo',
				'path'      => 'app/logo',
				'default'   => config('icetea.core.logo'),
				'width'     => 454,
				'height'    => 80,
				'upsize'    => true,
				'quality'   => 100,
		...
		if (isset($value['favicon'])) {
			$favicon = [
				'attribute' => 'favicon',
				'path'      => 'app/ico',
				'default'   => config('icetea.core.favicon'),
				'width'     => 32,
				'height'    => 32,
				'upsize'    => false,
				'quality'   => 100,
    ...
    {"name":"max_file_size","label":"Upload Max File Size","type":"text","hint":"Upload Max File Size (in KB)","wrapperAttributes":{"class":"form-group col-md-6"}}';
		...
		if ($this->key == 'geo_location') {
			$value = '{"name":"geolocation_activation","label":"Enable Geolocation","type":"checkbox","hint":"Before enabling this option you need to download the Maxmind database.","wrapperAttributes":{"class":"form-group col-md-6","style":"margin-top: 20px;"}},
    ...
    {"name":"separator_1_1","type":"custom_html","value":"Create a Facebook App <a href=\"https://developers.facebook.com/\" target=\"_blank\">here</a>. The \"OAuth redirect URI\" is: <strong>'.env('APP_URL').'/auth/facebook/callback</strong>"},
    ...
    {"name":"separator_2_1","type":"custom_html","value":"Create a Google+ App <a href=\"https://console.developers.google.com/\" target=\"_blank\">here</a>. The \"Authorized Redirect URI\" is: <strong>'.env('APP_URL').'/auth/google/callback</strong>"},
    ...
    {"name":"separator_4","type":"custom_html","value":"<h3>Mobile Apps URLs</h3>"},
    ...
    {"name":"separator_5","type":"custom_html","value":"<h3>Number Format</h3>"},
    {"name":"decimals_superscript","label":"Decimals Superscript","type":"checkbox"},
    
    {"name":"separator_6","type":"custom_html","value":"<h3>Optimization</h3>"},
    ...
    {"name":"separator_7","type":"custom_html","value":"<h3>JavaScript (in the &lt;head&gt; section)</h3>"},
    ...
    {"name":"separator_1_1","type":"custom_html","value":"You need to add \'/usr/bin/php -q /path/to/your/website/artisan ads:clean\' in your Cron Job tab."},
