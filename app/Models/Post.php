...
class Post extends BaseModel implements Feedable
{
    ...
    */
    ...
    public function getVipHtml()
    {
        return ajaxCheckboxDisplay($this->{$this->primaryKey}, $this->getTable(), 'vip', $this->vip);
    }
    ...
    */
    ...
    {
        return localUrl($this->country_code, $this->uri) . '?preview=1';
    }

    public function getUrl()
    {
        return localUrl($this->country_code, $this->uri);
    }

    public function getSalary()
    {
        $money = '';
        if ($this->salary_min > 0 or $this->salary_max > 0) {
            if ($this->salary_max > 0) {
                if ($this->salary_min > 0) {
                    $money .= ' - ';
                }

                $money .= \App\Helpers\Number::money($this->salary_max);
