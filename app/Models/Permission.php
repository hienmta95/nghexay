...
class Permission extends OriginalPermission
{
	...
	*/
	...
	{
		if (!config('icetea.admin.allow_permission_create')) {
			return null;
		}
	...
	{
		$permissions = self::getRoutesPermissions();
				...
				if (!empty($filePath) && file_exists($filePath)) {
					...
					if (str_contains($content, 'extends PanelController')) {
						...
						if (str_contains($controllerActionPath, '\PermissionController')) {
							...
							if (!config('icetea.admin.allow_permission_update')) {
								$denyAccess[] = 'update';
							}
							if (!config('icetea.admin.allow_permission_delete')) {
								$denyAccess[] = 'delete';
							}
						} else if (str_contains($controllerActionPath, '\RoleController')) {
							if (!config('icetea.admin.allow_role_create')) {
								$denyAccess[] = 'create';
							}
							if (!config('icetea.admin.allow_role_update')) {
								$denyAccess[] = 'update';
							}
							if (!config('icetea.admin.allow_role_delete')) {
								$denyAccess[] = 'delete';
							}
						} else {
