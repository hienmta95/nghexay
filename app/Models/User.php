...
{
    use Crud, HasRoles, CountryTrait, HasApiTokens, Notifiable, EloquentJoin;
    ...
    protected $appends = ['created_at_ta', 'hash'];
    ...
    */
    ...
    public function getAvatar($dimension = 200, $fontSize = 68)
    {
        $file = 'avatars/' . md5($this->id) . '.png';
        $path = \Storage::path($file);

        if (!file_exists($path) || $this->avatar == '') {
            Avatar::create($this->name)->setDimension($dimension)->setFontSize($fontSize)->save($path);
            $this->avatar = $file;
            $this->save();
        }
    ...
    */
    ...
    {
        return route('search.candidate.profile', $this->hash);
    }
