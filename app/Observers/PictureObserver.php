...
class PictureObserver
{
        ...
        if (!empty($picture->filename)) {
            ...
            if (!empty($files)) {
                foreach($files as $file) {
                    ...
                    if (str_contains($file, config('icetea.core.picture.default'))) {
                        continue;
                    }
	...
	private function clearCache($picture)
	{
		...
		Cache::forget('post.withoutGlobalScopes.with.user.city.pictures.' . $picture->post_id . '.' . config('app.locale'));
		Cache::forget('post.with.user.city.pictures.' . $picture->post_id . '.' . config('app.locale'));
	}
}
