...
trait EditTrait
{
	...
	public function postUpdateForm($postIdOrToken, PostRequest $request)
	{
		...
		//$post->deadline = $request->input('deadline') ? \Carbon\Carbon::parse($request->input('deadline')) : '';
		//$post->start_date = $request->input('start_date') ? \Carbon\Carbon::parse($request->input('start_date')) : '';
