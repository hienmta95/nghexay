...
class PostsController extends AccountBaseController
{
    ...
    public function getFavouritePosts()
    {
        ...
        MetaTag::set('description', t('My favourite jobs on :app_name', ['app_name' => config('settings.app.app_name')]));
     ...
     * @return View
     */
    public function getSuitablePosts()
    {
        $data = [];
        ...
        $data['posts'] = Post::where('category_id', $categoryId)->where('reviewed', 1)->where('archived', 0)->orderBy('updated_at', 'DESC')->paginate(20);

        // Meta Tags
        MetaTag::set('title', t('My suitable jobs'));
        MetaTag::set('description', t('My suitable jobs on :app_name', ['app_name' => config('settings.app.app_name')]));

        return view('account.suitable-posts', $data);
    }
