...
{
    $path = str_replace(url(config('icetea.admin.route_prefix', 'admin')), '', $path);
    $path = ltrim($path, '/');

    if (!empty($path)) {
        $path = config('icetea.admin.route_prefix', 'admin') . '/' . $path;
    } else {
        $path = config('icetea.admin.route_prefix', 'admin');
    }
...
function getArticleUrl($article)
{
    ...
    $url = localUrl($article->country_code, 'cam-nang/d-'.$article->getSlug());
    $out = '<a href="' . $url . '" target="_blank">' . str_limit($article->title, 60) . '</a>';
    return $out;
}
