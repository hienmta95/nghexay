...
function lurl($path = null, $attributes = [], $locale = null)
{
    ...
    return \Icetea\LaravelLocalization\Facades\LaravelLocalization::getLocalizedURL($locale, $path, $attributes);
}
...
{
    return getDomain() == config('icetea.core.demo.domain') || in_array(getHost(), (array)config('icetea.core.demo.hosts'));
}
        ...
        if (auth()->check()) {
            if (isFromAdminPanel()) {
                ...
                if (
                    ...
                    !str_contains(\Route::currentRouteAction(), 'Account\CompanyController') &&
                    !str_contains(\Route::currentRouteAction(), 'Account\ConversationsController') &&
                    !str_contains(\Route::currentRouteAction(), 'Account\ResumeController') &&
...
function resize($pathFromDb, $type = 'big')
{
    ...
    if (str_contains($pathFromDb, config('icetea.core.picture.default'))) {
        return \Storage::url($pathFromDb) . getPictureVersion();
    }

    // Get size dimensions
    $size = config('icetea.core.picture.resize.' . $type, '816x460');
    ...
    if (\Storage::exists($filepath . $thumbFilename)) {
        ...
        try {
            ...
            if (in_array($type, ['logo'])) {
                ...
                // Fit small pictures
                $image = \Image::make(\Storage::get($pathFromDb))->fit($width, $height)->encode($extension, config('icetea.core.picture.quality', 100));
            }
        } catch (\Exception $e) {
            return \Storage::url($pathFromDb) . getPictureVersion();
        }
...
if (!function_exists('resizeThumb')) {
    function resizeThumb($pathFromDb, $width = 200, $height = 200, $fill = 'false', $extract = 'false', $fillColor = 'FFFFFF', $stretch = 'false', $quality = 100)
    {
        $url = \Storage::url($pathFromDb);
        return $url . '?resize=w[' . $width . ']h[' . $height . ']f[' . $fill . ']e[' . $extract . ']fc[' . $fillColor . ']s[' . $stretch . ']q[' . $quality . ']';
...
{
    $pictureVersion = '';
    if (config('icetea.core.picture.versioned') && !empty(config('icetea.core.picture.version'))) {
        $pictureVersion = '?v=' . config('icetea.core.picture.version');
    }
...
function isEnabledField($field)
{
    // Front Register Form
    if ($field == 'phone') {
        return !config('icetea.core.disable.phone');
    } else if ($field == 'email') {
        return !config('icetea.core.disable.email') or
            (config('icetea.core.disable.email') and config('icetea.core.disable.phone'));
    } else if ($field == 'username') {
        return !config('icetea.core.disable.username');
    } else {
        return true;
    }
}
...
function lengthPrecision($number, $countryCode = null)
{
    ...
    $mileUseCountries = (array)config('icetea.core.mileUseCountries');
