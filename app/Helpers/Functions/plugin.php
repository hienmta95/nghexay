...
function plugin_list($category = null, $checkInstalled = false)
{
    ...
    $list = \File::glob(config('icetea.core.plugin.path') . '*', GLOB_ONLYDIR);
...
function plugin_namespace($pluginFolderName, $localNamespace = null)
{
    if (!is_null($localNamespace)) {
        return config('icetea.core.plugin.namespace') . $pluginFolderName . '\\' . $localNamespace;
    } else {
        return config('icetea.core.plugin.namespace') . $pluginFolderName;
    }
}
...
{
    return config('icetea.core.plugin.path') . $pluginFolderName . '/' . $localPath;
}
...
{
    $fullPath = config('icetea.core.plugin.path') . $pluginFolderName . '/' . $path;
...
function plugin_purchase_code_data($plugin, $purchaseCode)
{
    ...
    /*$apiUrl = config('icetea.core.purchaseCodeCheckerUrl') . $purchaseCode . '&item_id=' . $plugin->item_id;
    ...
    $data = json_decode('
                    {
                        "valid":"true",
                        "message":"valid",
                        "license_code":"11001000-0000-0000-0000-000000000000"
                    }');
    if ($data->valid == true) {
