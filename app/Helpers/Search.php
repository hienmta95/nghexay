...
class Search
{
    ...
    public function __construct($preSearch = [])
    {
        ...
        //$this->arrSql->join[] = "LEFT JOIN " . DBTool::table('companies') . " as cm ON cm.id=py.company_id";
        $this->arrSql->where = [
        ...
        // Priority setter
        if (request()->filled('distance') and is_numeric(request()->get('distance')) and request()->get('distance') > 0) {
    ...
    public function fetch()
    {
		...
		if (request()->filled('type') && !empty(request()->get('type'))) {
			$postTypeIds = (is_string(request()->get('type'))) ? [request()->get('type')] : request()->get('type');
			$cacheId = 'postTypes.' . md5(serialize($postTypeIds)) . '.' . config('app.locale');
			$postTypes = Cache::remember($cacheId, $this->cacheExpiration, function () use ($postTypeIds) {
			...
			if ($postTypes->count() <= 0) {
				$message = t('The requested job type does not exist.');
				if (is_array($postTypeIds) && count($postTypeIds) > 1) {
					$message = t('The requested job types do not exist.');
				}
				abort(404, $message);
			}
		}
    ...
    public function setLocationByCityCoordinates($lat, $lon, $cityId = null)
    {
		...
		$distanceCalculationFormula = config('icetea.core.distanceCalculationFormula');
