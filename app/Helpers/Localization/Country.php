...
class Country
{
	...
	// Maxmind Support URL
	private static $maxmindSupportUrl = '';
	...
	public function __construct()
	{
		...
		$this->defaultUrl = url(config('icetea.localization.default_uri'));
		$this->defaultPage = url(config('app.locale') . '/' . trans('routes.' . config('icetea.localization.countries_list_uri')));
	...
	public function setCountryParameters()
	{
		...
		if (
			in_array(getSegment(1), [
				trans('routes.' . config('icetea.localization.countries_list_uri')),
				'robots',
				'robots.txt',
				'feed',
			]) ||
