...
namespace Icetea\LaravelLocalization;
...
trait LocalizationTrait
{
	...
	public function getUrlThroughCurrentController($locale = null, $attributes = [])
	{
		...
		// Search: Category
		if (str_contains(Route::currentRouteAction(), 'Search\CategoryController@index')) {
		...
		else if (str_contains(Route::currentRouteAction(), 'Search\CityController@index')) {
			// Get the Route Path
		...
		else if (str_contains(Route::currentRouteAction(), 'Search\SearchController@index')) {
			// Get the Route Path
	...
	public function getUrlThroughEnteredRoute($locale = null, $url = null, $attributes = [])
	{
		...
		// Search: Category
		if (
			str_contains($url, trans('routes.t-search-cat', [], $locale))
			...
			str_contains($url, trans('routes.t-search-city', [], $locale))
			...
			str_contains($url, trans('routes.t-search-user', [], $locale))
			&& isset($attributes['id'])
			...
			str_contains($url, trans('routes.t-search-username', [], $locale))
			&& isset($attributes['id'])
			&& isset($attributes['username'])
		) {
			...
			str_contains($url, trans('routes.t-search-company', [], $locale))
			&& isset($attributes['id'])
		) {
			...
			str_contains($url, trans('routes.page', [], $locale))
			...
			str_contains($url, trans('routes.t-search', [], $locale))
			&& !str_contains($url, trans('routes.t-search-cat', [], $locale))
	...
	public static function getCategoryBySlug($slug, $locale)
	{
		...
		$tmpCat = Category::transIn(config('app.locale'))->where('slug', '=', $slug)->first();
		if (!empty($tmpCat)) {
