                    ...
                    @if (isset($savedSearch) and $savedSearch->getCollection()->count() > 0)
                        @foreach ($savedSearch->getCollection() as $search)
                            <li class="list-group-item {{ (Request::get('q')==$search->keyword) ? 'active' : '' }}">
                                <a href="{{ lurl('account/saved-search/?'.$search->query.'&pag='.Request::get('pag')) }}"
                                   class="">
                                    <span> {{ str_limit(strtoupper($search->keyword), 20) }} </span>
                                    <span class="label label-warning" id="{{ $search->id }}">{{ $search->count }}
                                        +</span>
                                </a>
                                ...
                                <div class="col-sm-8 add-desc-box">
                                    <div class="ads-details">
                                        ...
                                        <span class="info-row">
															...
															<span class="date"><i
                                                                        class=" icon-clock"> </i> {{ $post->created_at }} </span>
                                            @if (!empty($postCat))
                                                - <span class="category">{{ $postCat->name }} </span>
                                            @endif
                                            @if (!empty($postCity))
                                                - <span class="item-location"><i
                                                            class="fa fa-map-marker"></i> {{ $postCity->name }}</span>
                                            @endif
														</span>
                                    </div>
                                </div>
